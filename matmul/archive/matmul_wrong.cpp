...
static INLINE void kernel(
  ...
  alignas(64) static volatile f32 dummy[16];

  const volatile f32 *a0 = &dummy[0];
  const volatile f32 *a1 = &dummy[0];
  const volatile f32 *a2 = &dummy[0];
  const volatile f32 *a3 = &dummy[0];

  for (u32 k = l; k < r; k++) {
    f32x16 b0 = f32x16{} + dummy[0]; // b[(k * n + y) / 16 + 0];
    f32x16 b1 = f32x16{} + dummy[0]; // b[(k * n + y) / 16 + 1];

  #define FMADD(i)\
    f32x16 _a##i = f32x16{} + *(a##i);\
    d##i##0 += _a##i * b0;\
    d##i##1 += _a##i * b1;
